...
%       spect_store_300(:,1) and spect_store_300(:,2)
...
%   - Sample #2: "Light sponge" (parameter vector m2)
...
%
% Priors 
%   - phi (sample1): Normal(mean=0.05, sd=0.01)
...
addpath('/Users/kyuninkim/Downloads/Inv_labexp/Lib_exp');
addpath('/Users/kyuninkim/Downloads/Inv_labexp/file_store');

load('Spect_store_50_store.mat','f_h');                % frequency vector
load('spect_store_300_store.mat','spect_store_300');  % observed spectra

% Observed amplitude spectra (row vectors)
obs_amp_1 = spect_store_300(1,:);   % sample #1
obs_amp_2 = spect_store_300(2,:)t;   % sample #2

%% ---------------------- MCMC configuration ------------------------------
kk  = 41;        % run label for output files
global tau
...
L_1   = 0.20;     % sample #1 porous length (m)  (was L_val2)
...
kappa0   = -7.1;         % log10(permeability) for sample #1
kappa0_d = -7.1;         % log10(permeability) for sample #2
...
Q_ml_s = 250;             % initial flow in ml/s (for readability)
Q0     = Q_ml_s * 1.2e-6;
...
lp_phi_1 = @(phi) -(phi - 0.05)^2 / (2 * 0.01^2);
...
% likelihood and acceptance ratio store
...
for k = 1:niter
    ...
    if mod(k, 20000) == 0 || k == 100 || k == 1000
        ...
        set(gca,'YTickLabel',[]);
        saveas(h13,sprintf('MCMC_permeability_%d.jpg',kk))
        ...
        set(gca,'YTickLabel',[]);
         saveas(h14,sprintf('MCMC_sigma_%d.jpg',kk))
        ...
        saveas(h15,sprintf('MCMC_porosity_%d.jpg',kk))

        h16=figure(16); clf
        histogram(gasstore(1:k)/(1.2e-6), 'Normalization','Probability','NumBins',50);
        xlim([0 400]); xlabel('Gas flow rate (ml/s)'); title('Sample 1: gas flow');
        set(gca,'YTickLabel',[]);
        saveas(h16,sprintf('MCMC_gasflow_%d.jpg',kk))
        ...
        set(gca,'YTickLabel',[]);
        saveas(h18,sprintf('MCMC_permeability2_%d.jpg',kk))

        h19=figure(19); clf
        histogram(gasstore2(1:k)/(1.2e-6), 'Normalization','Probability','NumBins',50);
        xlim([0 400]); xlabel('Gas flow rate (ml/s)'); title('Sample 2: gas flow');
        set(gca,'YTickLabel',[]);
        saveas(h19,sprintf('MCMC_gasflow2_%d.jpg',kk))
        ...
        saveas(h22,sprintf('MCMC_porosity2_%d.jpg',kk))
        ...
        saveas(h23,sprintf('Fit_synthetic_%d.jpg',kk))
        ...
        saveas(h20,sprintf('Fit_synthetic2_%d.jpg',kk))
