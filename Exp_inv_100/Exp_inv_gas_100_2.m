...
%   - Sample #1: "9 capillary" 
%   - Sample #2: "Light sponge" 
...
load('spect_store_100_store.mat','spect_store_100');  % observed spectra

% Observed amplitude spectra (row vectors)
obs_amp_1 = spect_store_100(:,3)';   % sample #1
obs_amp_2 = spect_store_100(:,4)';   % sample #2

%% ---------------------- MCMC configuration ------------------------------
kk  = 21;        % run label for output files
global tau
...
sigma0   = 0.011;        % initial impulse timescale for sample #1 (s)
sigma0_d = 0.011;        % initial impulse timescale for sample #2 (s)
...
for k = 1:niter
    ...
    if mod(k, 20000) == 0 || k == 100 || k == 1000
        ...
        saveas(h24,sprintf('Fit_synthetic_%d.jpg',kk))
